...
from clawpack.geoclaw import fgout_tools, util
from clawpack.visclaw import animation_tools, geoplot, gridtools, plottools
...
clim(-0.3, 0.3)
cb = colorbar(
    eta_plot, extend="both", shrink=0.5, orientation="horizontal", anchor=(0.4, 1)
)
cb.set_label("meters")
title_text = title("Surface at time %s" % timedelta(seconds=fgout.t))

ax.set_aspect(1.0 / cos(ylat * pi / 180.0))
...
def extract_transect(fgout_soln, xtrans, ytrans):
    ...
    eta1d = gridtools.grid_eval_2d(
        fgout_soln.X, fgout_soln.Y, fgout_soln.eta, xtrans, ytrans, method="nearest"
    )
    B1d = gridtools.grid_eval_2d(
        fgout_soln.X, fgout_soln.Y, fgout_soln.B, xtrans, ytrans, method="nearest"
    )
...
axtrans1 = axes([0.55, 0.6, 0.4, 0.3])
...
axtrans2 = axes([0.55, 0.1, 0.4, 0.3])
...
# filled regions:
Bfill_plot = axtrans2.fill_between(
    xtrans, Btrans - 1e5, Btrans, color=[0.7, 1, 0.7, 1]
)  # light green
etafill_plot = axtrans2.fill_between(
    xtrans, Btrans, etatrans, color=[0.7, 0.7, 1, 1]
)  # light blue
...
def update(fgframeno):
    ...
    dummy = axdummy.fill_between(xtrans, Btrans - 1e5, Btrans, color=[0.5, 1, 0.5, 1])
    dp = dummy.get_paths()[0]
    dummy.remove()
    Bfill_plot.set_paths([dp.vertices])

    dummy = axdummy.fill_between(xtrans, Btrans, etatrans, color=[0.5, 0.5, 1, 1])
    dp = dummy.get_paths()[0]
    dummy.remove()
...
if __name__ == "__main__":
    ...
    anim = animation.FuncAnimation(
        fig, update, frames=fgframes, interval=200, blit=False
    )
