    ...
    *TODO:*  Add description of unit handling
    ...
    def read_geoclaw(self, path, verbose=False):
        ...
        with open(path, "r") as data_file:
            ...
            try:
                self.time_offset = datetime.datetime.strptime(time, "%Y-%m-%dT%H:%M:%S")
            except ValueError:
    ...
    def read_atcf(self, path, verbose=False):
        ...
        df = pd.read_csv(
            ...
            dtype={"BASIN": str, "CY": int, "VMAX": float, "MSLP": float, "TY": str},
        )
    ...
    def read_ibtracs_processed(self, path, sid, start_date=None):
        ...
        with xr.open_dataset(path, engine=engine, backend_kwargs=backend_kwargs) as ds:
            ...
            # set landfall events
            self.event = np.array([""] * len(ds.datetime))
            ...
            self.eye_location = np.array([ds.longstore, ds.latstore]).T
    ...
    def read_emanuel(self, path, storm_index, ensemble=None, verbose=False):
        ...
        with xr.open_zarr(path, drop_variables=["time"], chunks=None) as ds:
            ...
            self.event = np.empty(num_timesteps, dtype=str)
            self.classification = np.empty(num_timesteps, dtype=str)
    ...
    def write_geoclaw(
        self, path, force=False, skip=True, verbose=False, fill_dict={}, **kwargs
    ):
            ...
            for name in [
                ...
                if np.isnan(getattr(self, name)[n]):
                    if name in fill_dict.keys():
                        ...
                        msg = (
                            "The value {} at {} is a NaN and the storm "
    ...
    def plot(
        ...
        if isinstance(track_style, str):
            ...
            if self.max_wind_speed is None:
                raise ValueError(
                    "Maximum wind speed not available so "
            ...
            colors = [
                track_style.get(category, cat_color_defaults[category])
        ...
        # Plot swath
        if (
            isinstance(radius, float)
            or isinstance(radius, np.ndarray)
            or radius is None
        ):
            if radius is None:
                # Default behavior
                if self.storm_radius is None:
                    raise ValueError(
                        "Cannot use storm radius for plotting "
                    ...
                    if coordinate_system == 1:
                        ...
                        _radius = units.convert(self.storm_radius, "m", "lat-long")
                    else:
                        raise ValueError(
                            f"Unknown coordinate system "
                ...
                raise ValueError(
                    "Invalid input argument for radius.  Should " "be a float or None"
                )

            # Draw first and last points
            ax.add_patch(plt.Circle((x[0], y[0]), _radius[0], color=fill_color))
            if t.shape[0] > 1:
                ax.add_patch(plt.Circle((x[-1], y[-1]), _radius[-1], color=fill_color))
