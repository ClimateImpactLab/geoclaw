    ...
    subroutine set_holland_1980_fields(maux, mbc, mx, my, xlower, ylower,    &
        ...
        do j=1-mbc,my+mbc
            ...
            do i=1-mbc,mx+mbc
                ...
                IF (inv_rel_dist > 10) THEN

                    print *, r, mwr

                    aux(pressure_index,i,j) = Pc
                ELSE
                    aux(pressure_index,i,j) = Pc + dp * exp(-inv_rel_dist**B)
                END IF

                ! Speed of wind at this point

                ! use a threshold of 20 to prevent underflow - higher due to
                ! compensating increasing and decreasing effects
                IF (inv_rel_dist > 20) THEN
                    wind = sqrt((r * f)**2.d0 / 4.d0) - r * f / 2.d0
                ELSE
                    wind = sqrt(inv_rel_dist**B &
                            * exp(1.d0 - inv_rel_dist**B) * mws**2.d0 &
